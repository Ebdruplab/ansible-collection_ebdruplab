...
options:
  ...
  schedule:
    type: dict
    required: true
    description: Updated schedule properties
  session_cookie:
    type: str
    required: false
    no_log: true
...
def main():
    ...
    base_url = f"{host}:{port}/api/project/{project_id}/schedules/{schedule_id}"
    ...
    try:
        ...
        module.exit_json(changed=True, updated=True, schedule=schedule_obj, status=status)
