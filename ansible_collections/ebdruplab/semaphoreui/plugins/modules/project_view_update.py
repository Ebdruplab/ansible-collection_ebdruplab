...
def main():
    ...
    try:
        current_view, resp_body, get_status, _ = semaphore_get_json(
            url=url,
            headers=headers,
            validate_certs=validate_certs,
        )
        if get_status != 200 or not isinstance(current_view, dict):
            module.fail_json(
                msg=f"Failed to fetch current view state: HTTP {get_status}",
                status=get_status,
        ...
        if not changed:
            module.exit_json(
                changed=False,
                view=sanitize_check_mode_value(current_view),
                status=200,
            )
