...
EXAMPLES = r"""
- name: List integration matchers
...
def main():
    ...
    try:
        ...
        if status != 200:
            ...
            module.fail_json(msg=f"Failed to list matchers: HTTP {status} - {text}", status=status)
        ...
        module.exit_json(changed=False, matchers=data, status=status)
