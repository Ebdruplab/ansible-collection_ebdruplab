...
def main():
    ...
    try:
        ...
        current_inventory, response_body, get_status, _ = semaphore_get_json(
            url,
            headers=headers,
            validate_certs=module.params["validate_certs"]
        )
        if get_status != 200 or not isinstance(current_inventory, dict):
            module.fail_json(
                msg=f"Failed to fetch current inventory state: HTTP {get_status}",
                status=get_status,
                response=response_body,
            )

        before = {
            key: current_inventory.get(key)
        ...
        if not changed:
            module.exit_json(
                changed=False,
                inventory=sanitize_check_mode_value(current_inventory),
                status=200,
            )
